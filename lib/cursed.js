...
cursed.Worker = function(host, port, task_module) {
    this.server = cursed.Server(host, port);
...
cursed.Cluster = function (concurrency, task_module, ip, port) {
    ...
    var create_worker = function (ip, port) {
        ...
        child.stdout.addLister('data', function (data) {
            log(data);
        });

        child.stderr.addLister('data', function (data) {
            log('ERROR: ' + data);
        });
    }
    ...
    this.workers = []
    for(var i =0; i < concurrency; i++) this.workers.push(create_server(ip, port + i));
}
